...
export default function ComputeCalc({ th, isMobile=false }) {
  ...
  const existingGhz = existing.cores * existing.ghz;
  const targetGhz = target.cores * target.ghz;
  const n1Servers = Math.max(target.servers - target.haLoss, 1);
  const n1Ratio = n1Servers / target.servers;
  const n1Cores = Math.round(target.cores * n1Ratio);
  const n1Ram = Math.round(target.ram * n1Ratio);
  const n1Ghz = Math.round(targetGhz * n1Ratio);

  const cpuGain = existing.cores ? Math.round(((target.cores - existing.cores) / existing.cores) * 100) : 0;
  ...
  const cpuOkN1 = n1Cores >= existing.cores * (1 + target.growth / 100);
  ...
  const Bar = ({label,oldv,newv,n1v,unit}) => {
    const max = Math.max(oldv,newv,n1v,1);
    return (
      <div style={{marginBottom:16}}>
        <div style={{fontSize:12,fontWeight:800,color:th.t1,marginBottom:8}}>{label}</div>
        {[
          ["Existant", oldv, "#9ca3af"],
          ["Cible", newv, th.accent2],
          ["N-1", n1v, th.accent],
        ].map(([n,v,c])=>(
          <div key={n} style={{display:"grid",gridTemplateColumns:"70px 1fr 70px",gap:8,alignItems:"center",marginBottom:6}}>
            <div style={{fontSize:11,color:th.t2}}>{n}</div>
            <div style={{height:9,borderRadius:99,background:th.bg2,overflow:"hidden"}}>
              <div style={{height:"100%",width:`${Math.round((v/max)*100)}%`,background:c,borderRadius:99}}/>
            </div>
            <div style={{fontSize:11,fontWeight:900,color:c,textAlign:"right"}}>{fmt(v)} {unit}</div>
          </div>
        ))}
      </div>
    );
        ...
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:18}}>
          <Card>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              ...
              <Field title="Cœurs CPU totaux" value={existing.cores} onChange={v=>setExisting({...existing,cores:v})}/>
              <Field title="Fréquence moyenne GHz" step={0.1} value={existing.ghz} onChange={v=>setExisting({...existing,ghz:v})}/>
              <Field title="RAM totale Go" value={existing.ram} onChange={v=>setExisting({...existing,ram:v})}/>
            </div>
          </Card>
          ...
          <Card>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              ...
              <Field title="Cœurs CPU cible" value={target.cores} onChange={v=>setTarget({...target,cores:v})}/>
              <Field title="Fréquence CPU GHz" step={0.1} value={target.ghz} onChange={v=>setTarget({...target,ghz:v})}/>
              <Field title="RAM cible Go" value={target.ram} onChange={v=>setTarget({...target,ram:v})}/>
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(5,1fr)",gap:14,marginBottom:18}}>
        ...
        <Kpi title="Cœurs cible" value={fmt(target.cores)} sub={`+${cpuGain}% vs existant`} color={th.accent2} icon={<Cpu size={28}/>}/>
        <Kpi title="GHz agrégés" value={fmt(targetGhz)} sub={`+${ghzGain}% vs existant`} color="#3b82f6" icon={<Gauge size={28}/>}/>
      ...
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"2fr 1fr",gap:18}}>
        <Card>
          ...
          <div style={{display:"grid",gridTemplateColumns:"160px repeat(3,1fr)",border:`1px solid ${th.border}`,borderRadius:12,overflow:"hidden"}}>
            ...
            {[
              ...
              ["Cœurs CPU", existing.cores, target.cores, n1Cores],
              ["GHz agrégés", Math.round(existingGhz), Math.round(targetGhz), n1Ghz],
        ...
        <Card>
          <div style={{fontSize:16,fontWeight:950,color:th.t1,marginBottom:16}}>Comparaison & gains</div>
          <Bar label="Cœurs CPU" oldv={existing.cores} newv={target.cores} n1v={n1Cores} unit=""/>
          <Bar label="GHz agrégés" oldv={Math.round(existingGhz)} newv={Math.round(targetGhz)} n1v={n1Ghz} unit=""/>
          <Bar label="RAM totale" oldv={existing.ram} newv={target.ram} n1v={n1Ram} unit="Go"/>
          <Bar label="Nœuds" oldv={existing.servers} newv={target.servers} n1v={n1Servers} unit=""/>
        </Card>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr 1fr",gap:18,marginTop:18}}>
        ...
        <Card>
          ...
          <div style={{fontSize:13,color:th.t2,lineHeight:1.6}}>
            La cible apporte +{cpuGain}% de cœurs CPU et +{ramGain}% de RAM par rapport à l’existant.
          </div>
        </Card>
      </div>
    </div>
