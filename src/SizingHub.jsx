...
function ComputeCalc({ th, isMobile=false }) {
  ...
  // ── Mode simplifié Sizing Rapide ──
  const [srHypervisor,  setSrHypervisor]  = useState('vmware');
  const [srStorage,     setSrStorage]     = useState('ceph-rf2');
  const [srSockets,     setSrSockets]     = useState(2);
  const [srNodes,       setSrNodes]       = useState(3);
  const [srVcpu,        setSrVcpu]        = useState(200);
  const [srOvercommit,  setSrOvercommit]  = useState(4);
  const [srRam,         setSrRam]         = useState(512);
  const [srStorage2,    setSrStorage2]    = useState(20);

  const sr = useMemo(()=>{
    const haNodes = srNodes - 1;
    if(haNodes <= 0) return null;
    // CPU
    const physCoresNeeded = Math.ceil(srVcpu / srOvercommit);
    const physCoresPerNode = Math.ceil(physCoresNeeded / haNodes);
    const coreStd = [16,24,32,48,64,96];
    const recoCores = coreStd.find(c => c >= physCoresPerNode) || 96;
    // RAM
    const ramPerNode = Math.ceil(srRam / haNodes);
    const ramStd = [64,128,192,256,384,512,768,1024,1536,2048];
    const recoRam = ramStd.find(r => r >= ramPerNode) || 2048;
    // Stockage
    let storagePerNode;
    let storageWarn = null;
    if(srHypervisor === 'proxmox' && srStorage === 'zfs') {
      storagePerNode = Math.ceil(srStorage2 * 1.2);
      if(srNodes > 2) storageWarn = 'info';
    } else if(srHypervisor === 'proxmox' && srStorage === 'ceph-rf2') {
      storagePerNode = Math.ceil((srStorage2 * 2) / srNodes);
      if(srNodes < 3) storageWarn = 'quorum';
    } else if(srHypervisor === 'proxmox' && srStorage === 'ceph-rf3') {
      storagePerNode = Math.ceil((srStorage2 * 3) / srNodes);
      if(srNodes < 3) storageWarn = 'quorum';
    } else {
      storagePerNode = Math.ceil((srStorage2 * 1.2) / haNodes);
    }
    return { recoCores, recoRam, storagePerNode, physCoresNeeded, physCoresPerNode, ramPerNode, storageWarn };
  }, [srHypervisor, srStorage, srSockets, srNodes, srVcpu, srOvercommit, srRam, srStorage2]);
  ...
  return (
    <div>
      {/* KPIs colorés */}
      {/* ── Mode simplifié Sizing Rapide ── */}
      <div style={{marginBottom:20,borderRadius:8,border:`1px solid ${th.border}`,overflow:'hidden'}}>
        <div style={{background:'linear-gradient(135deg,#0077cc,#005599)',padding:'10px 16px'}}>
          <span style={{fontSize:11,fontWeight:600,color:'#fff',textTransform:'uppercase',letterSpacing:'0.08em',fontFamily:'monospace'}}>Sizing Rapide — Recommandation par noeud</span>
        </div>
        <div style={{padding:16}}>
          {/* Inputs */}
          <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)',gap:10,marginBottom:14}}>
            <div>
              <label style={{display:'block',fontSize:10,color:th.t3,fontFamily:'monospace',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:5}}>Hyperviseur</label>
              <select value={srHypervisor} onChange={e=>setSrHypervisor(e.target.value)} style={{width:'100%',background:th.bg2,border:`1px solid ${th.border}`,borderRadius:4,padding:'7px 10px',color:th.t1,fontFamily:'monospace',fontSize:12}}>
                <option value='vmware'>VMware VVF/VCF</option>
                <option value='nutanix'>Nutanix AHV</option>
                <option value='hyperv'>Hyper-V</option>
                <option value='proxmox'>Proxmox</option>
              </select>
            </div>
            {srHypervisor==='proxmox'&&(
              <div>
                <label style={{display:'block',fontSize:10,color:th.t3,fontFamily:'monospace',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:5}}>Stockage</label>
                <select value={srStorage} onChange={e=>setSrStorage(e.target.value)} style={{width:'100%',background:th.bg2,border:`1px solid ${th.border}`,borderRadius:4,padding:'7px 10px',color:th.t1,fontFamily:'monospace',fontSize:12}}>
                  <option value='zfs'>ZFS + Réplication</option>
                  <option value='ceph-rf2'>Ceph RF2</option>
                  <option value='ceph-rf3'>Ceph RF3</option>
                </select>
              </div>
            )}
            <div>
              <label style={{display:'block',fontSize:10,color:th.t3,fontFamily:'monospace',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:5}}>Processeurs</label>
              <select value={srSockets} onChange={e=>setSrSockets(Number(e.target.value))} style={{width:'100%',background:th.bg2,border:`1px solid ${th.border}`,borderRadius:4,padding:'7px 10px',color:th.t1,fontFamily:'monospace',fontSize:12}}>
                <option value={1}>Mono-processeur</option>
                <option value={2}>Bi-processeur</option>
              </select>
            </div>
            <div>
              <label style={{display:'block',fontSize:10,color:th.t3,fontFamily:'monospace',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:5}}>Nombre de noeuds</label>
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                <input type='range' min={2} max={32} value={srNodes} onChange={e=>setSrNodes(Number(e.target.value))} style={{flex:1}}/>
                <span style={{fontSize:13,fontWeight:600,fontFamily:'monospace',color:th.t1,minWidth:30}}>{srNodes}</span>
              </div>
            </div>
            <div>
              <label style={{display:'block',fontSize:10,color:th.t3,fontFamily:'monospace',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:5}}>vCPUs cible cluster</label>
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                <input type='number' min={10} max={10000} step={10} value={srVcpu} onChange={e=>setSrVcpu(Number(e.target.value))} style={{width:'100%',background:th.bg2,border:`1px solid ${th.border}`,borderRadius:4,padding:'7px 10px',color:th.t1,fontFamily:'monospace',fontSize:13}}/>
                <span style={{fontSize:11,color:th.t3}}>vCPU</span>
              </div>
            </div>
            <div>
              <label style={{display:'block',fontSize:10,color:th.t3,fontFamily:'monospace',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:5}}>Overcommit vCPU</label>
              <select value={srOvercommit} onChange={e=>setSrOvercommit(Number(e.target.value))} style={{width:'100%',background:th.bg2,border:`1px solid ${th.border}`,borderRadius:4,padding:'7px 10px',color:th.t1,fontFamily:'monospace',fontSize:12}}>
                {[[1,'1:1 — Sans overcommit'],[2,'2:1'],[3,'3:1'],[4,'4:1 — Standard'],[6,'6:1'],[8,'8:1 — Densité élevée']].map(([v,l])=><option key={v} value={v}>{l}</option>)}
              </select>
            </div>
            <div>
              <label style={{display:'block',fontSize:10,color:th.t3,fontFamily:'monospace',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:5}}>RAM cible cluster (Go)</label>
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                <input type='number' min={64} max={102400} step={64} value={srRam} onChange={e=>setSrRam(Number(e.target.value))} style={{width:'100%',background:th.bg2,border:`1px solid ${th.border}`,borderRadius:4,padding:'7px 10px',color:th.t1,fontFamily:'monospace',fontSize:13}}/>
                <span style={{fontSize:11,color:th.t3}}>Go</span>
              </div>
            </div>
            <div>
              <label style={{display:'block',fontSize:10,color:th.t3,fontFamily:'monospace',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:5}}>Stockage utile cible (To)</label>
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                <input type='number' min={1} max={1000} step={1} value={srStorage2} onChange={e=>setSrStorage2(Number(e.target.value))} style={{width:'100%',background:th.bg2,border:`1px solid ${th.border}`,borderRadius:4,padding:'7px 10px',color:th.t1,fontFamily:'monospace',fontSize:13}}/>
                <span style={{fontSize:11,color:th.t3}}>To</span>
              </div>
            </div>
          </div>
          {/* Warnings */}
          {sr&&sr.storageWarn==='quorum'&&<div style={{background:'rgba(255,181,71,0.1)',border:'1px solid rgba(255,181,71,0.3)',borderRadius:4,padding:'8px 12px',marginBottom:10,fontSize:11,color:'#ffb347',fontFamily:'monospace'}}>
            Ceph necessite minimum 3 noeuds pour le quorum
          </div>}
          {sr&&sr.storageWarn==='info'&&<div style={{background:'rgba(0,153,255,0.08)',border:'1px solid rgba(0,153,255,0.2)',borderRadius:4,padding:'8px 12px',marginBottom:10,fontSize:11,color:th.accent2,fontFamily:'monospace'}}>
            ZFS + replication est optimal pour 2 noeuds — considerez Ceph pour 3+ noeuds
          </div>}
          {sr&&srNodes<=2&&srHypervisor!=='proxmox'&&<div style={{background:'rgba(255,181,71,0.1)',border:'1px solid rgba(255,181,71,0.3)',borderRadius:4,padding:'8px 12px',marginBottom:10,fontSize:11,color:'#ffb347',fontFamily:'monospace'}}>
            2 noeuds — aucune tolerance a la panne en N+1
          </div>}
          {/* KPIs recommandation */}
          {sr&&<div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)',gap:10,marginBottom:10}}>
            <div style={{background:'linear-gradient(135deg,#e05a20,#b84510)',borderRadius:8,padding:'14px 16px'}}>
              <div style={{fontSize:10,color:'rgba(255,255,255,0.6)',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:2}}>CPU / noeud</div>
              <div style={{fontSize:10,color:'rgba(255,255,255,0.5)',marginBottom:4}}>{srSockets===1?'Mono':'Bi'}-proc · {sr.recoCores} coeurs/socket</div>
              <div style={{fontSize:22,fontWeight:700,fontFamily:'monospace',color:'#fff'}}>{srSockets*sr.recoCores}c</div>
            </div>
            <div style={{background:'linear-gradient(135deg,#5a4fcf,#3d35a0)',borderRadius:8,padding:'14px 16px'}}>
              <div style={{fontSize:10,color:'rgba(255,255,255,0.6)',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:2}}>RAM / noeud</div>
              <div style={{fontSize:10,color:'rgba(255,255,255,0.5)',marginBottom:4}}>Standard DDR5 · N+1</div>
              <div style={{fontSize:22,fontWeight:700,fontFamily:'monospace',color:'#fff'}}>{sr.recoRam} Go</div>
            </div>
            <div style={{background:'linear-gradient(135deg,#2d7a4f,#1a5c38)',borderRadius:8,padding:'14px 16px'}}>
              <div style={{fontSize:10,color:'rgba(255,255,255,0.6)',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:2}}>Stockage brut / noeud</div>
              <div style={{fontSize:10,color:'rgba(255,255,255,0.5)',marginBottom:4}}>{srHypervisor==='proxmox'?srStorage==='zfs'?'ZFS x1.2':srStorage==='ceph-rf2'?'Ceph RF2':'Ceph RF3':'Overhead x1.2 · N+1'}</div>
              <div style={{fontSize:22,fontWeight:700,fontFamily:'monospace',color:'#fff'}}>{sr.storagePerNode} To</div>
            </div>
          </div>}
          {/* Résumé */}
          {sr&&<div style={{background:th.bg2,borderRadius:4,padding:'10px 14px',fontSize:12,fontFamily:'monospace',color:th.t1}}>
            Recommandation : <strong>{srNodes} noeuds</strong> · {srSockets===1?'Mono':'Bi'}-proc {sr.recoCores}c · {sr.recoRam} Go RAM · {sr.storagePerNode} To brut
          </div>}
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(5,1fr)",gap:10,marginBottom:20}}>
